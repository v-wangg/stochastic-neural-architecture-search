  ...
  def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev):
    ...
    for i in range(self._steps):
      for j in range(2+i):
        ...
        op = MixedOp(C, stride) # Apply equation (2)
        self._ops.append(op)
...
class Network(nn.Module):
    ...
    def forward(self, input , temperature):
        ...
        for i, cell in enumerate(self.cells):  
            if cell.reduction:
                Z = self.ArchitectDist(self.alphas_reduce, temperature)
            else:
    ...
    def _loss(self, data, target, temperature):
        output, _ = self(data , temperature ) 
